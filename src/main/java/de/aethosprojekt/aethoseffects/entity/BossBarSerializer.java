...
public class BossBarSerializer {
    ...
    private static final String MAGIC = "ꈁꈇꈅꈄꈁ";

    public static @NotNull String serialize(ActiveEffect effect) {
        return MAGIC + effect.getType().getIcon() + String.format("%02d:%02d", effect.getRemainingTicks() / 1200, (effect.getRemainingTicks() % 1200) / 20);
    }
